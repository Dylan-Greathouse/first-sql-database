...
app.get('/videogames/:id', async(req, res) => {
  ...
  try {
    const data = await client.query('SELECT * from videogames WHERE id = $1', [id]);
...
app.post('/videogames', async(req, res) => {
  const { name, system, played, year_released } = req.body;
...
app.put('/videogames/:id', async(req, res) =>{
  const { id, name, system, played, year_released } = req.body;
  try{
    const videogamesData = await client.query(`
    UPDATE videogames
    SET
